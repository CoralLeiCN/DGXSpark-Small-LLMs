# 2026-09-05T23:09:10Z — Service removed despite shutdown cleanup traceback

Run ID: `RUN-0002`

- Status: workaround
- Phase: container shutdown
- Related turns: [Gemma E4B text API qualified after cold download](2026-09-05T23-07-59Z-text-api-qualified.md)
...
- Container: `dgxspark/gemma-4-e4b-it-sglang:0.1.0` (`sha256:326c8aa85e4ac705f9e7ed17ea19ac079851a56c97cfc87bb9a9715a968baf41`)
...
scripts/status google/gemma-4-e4b-it --engine sglang
...
- Symptom: normal Compose shutdown produced an NCCL cleanup warning and an
  application-level traceback after all requests had drained.
- Root cause: current hypothesis is cleanup ordering in this SGLang development
  ...
  successful `SystemExit: 0` while Starlette's lifespan task is cancelled; the
  process exits before PyTorch observes an explicit process-group destroy.
...
Gemma E4B container: stopped and removed
Gemma E4B endpoint: unavailable as expected
Retained volumes:
  dgxspark-gemma-4-e4b-it_sglang-cache
  dgxspark-gemma-4-e4b-it_triton-cache
```
